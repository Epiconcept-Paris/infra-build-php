#
#	debian/9/Dockervars.sh - Define Debian packages required in Dockerfile for build
#
BUILD_REQ="file curl bzip2 zip autoconf make patch build-essential pkg-config fakeroot dpkg-dev lintian libjpeg62-turbo libpng16-16 apache2 apache2-dev libmcrypt-dev libmhash-dev libxslt1-dev libxpm-dev libbz2-dev libfreetype6-dev default-libmysqlclient-dev libcurl4-openssl-dev libjpeg-dev libpng-dev libreadline6-dev libssl-dev libxml2-dev libxft-dev libpcre3-dev libc-dev vim"
TESTS_REQ="apache2 curl vim libjpeg62-turbo libpng16-16 libxpm4 libmcrypt4 libxslt1.1 libcurl3 libfreetype6 libmariadbclient18"
